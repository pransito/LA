...
do_maxim = function(cur_sub,mus,lambdas,x,fn.c) {
...
maxim_results <- foreach(ff=1:length(all_subs),.verbose=T) %dopar% {
  ...
  do_maxim(all_subs[ff],mus,lambdas,x,fn)
} 
stopCluster(cl)
...
# saving
save(file="Charpentier_model.RData",list = c("all_params","fn.c"))
} else {
  load("Charpentier_model.RData")
}
