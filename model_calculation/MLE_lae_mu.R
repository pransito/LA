...
fn<- function(theta,x){
  ...
  for (k in 1:nrow(x)){ #go through all trials
    if (is.na(x$accnum[k])){#skip trials without reaction
      ...
      value    = beta_int+x$gain[k]*beta_gain+x$loss[k]*beta_loss+x$ed_abs[k]*beta_ed;
...
do_maxim = function(cur_sub,mus,beta_ints,beta_gains,beta_losss,beta_eds,x,fn.c) {
  ...
  for (kk in 1:length(mus)) {
    cur_theta = c(mus[kk],beta_ints[kk],beta_gains[kk],beta_losss[kk],beta_eds[kk])
...
if (estimate_models == 1) {
  ...
  maxim_results <- foreach(ff=1:length(all_subs),.verbose=T) %dopar% {
    ...
    do_maxim(all_subs[ff],mus,beta_ints,beta_gains,beta_losss,beta_eds,x,fn)
  } 
  stopCluster(cl)
  ...
  # saving
  save(file="laem.RData",list = c("all_params","fn.c"))
} else {
  load("laem.RData")
}
