...
for (i in 1:length(all.behav.files)) {  
  ...
  df.cur.person$Gewinn_bb  = df.cur.person$Gewinn
  df.cur.person$Verlust_bb = df.cur.person$Verlust
  
  ## aggregate
  df.cur.person$Gewinn  = agk.aggregate.data.la.gain.loss(df.cur.person$Gewinn,agg=des_agg)
  df.cur.person$Verlust = agk.aggregate.data.la.gain.loss(df.cur.person$Verlust,agg=des_agg)
  
  ## euclidean distance ## NEED TO GET IT HERE; NOW IN MAJ REVISION NSR JUNE 2017 will delete it in big script ##
  ed <- c()
  
  for (g in 1:length(df.cur.person[,1])) {
    cur.ed  <- agk_get_ed(c(df.cur.person$Gewinn[g],abs(df.cur.person$Verlust[g]),0),sp = c(26,13,0),vec = c(2,1,0))
    ed      <- rbind(ed, as.numeric(cur.ed))
  }
  
  ed.abs <- ed
  df.cur.person <- data.frame(df.cur.person, ed.abs)
  ...
  ## die Formeln von Minati und Martino und mir sind hier gleich (ODER AUCH NICHT!!!)
  if (loss.abs == 1) {
  ...
  # scaling
  df.cur.person$Gewinn_bcp  <- df.cur.person$Gewinn
  df.cur.person$Verlust_bcp <- df.cur.person$Verlust
  df.cur.person$ratio_bcp   <- df.cur.person$ratio
